...
import { Shield, Search, BookOpen, HelpCircle, BarChart3, Eye, Camera, Accessibility } from 'lucide-react'
...
export default function Home() {
  ...
  const features = [
    ...
    { icon: Camera, title: 'AI Image Detector', description: 'Detect AI-generated photos from real photographs', link: '/image-detector' },
    ...
    { icon: Accessibility, title: 'Accessible for All', description: 'Voice control, sign language, and multilingual support', link: '/accessibility' },
  ]
    ...
    <div>
      <section className="bg-gradient-to-br from-primary-600 to-primary-800 text-white">
        <div className="max-w-7xl mx-auto px-4 py-24 text-center">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-xl text-primary-100 mb-8 max-w-2xl mx-auto">
            {t.home.subtitle}
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <Link to="/analyze" className="inline-block bg-white text-primary-700 font-semibold px-8 py-3 rounded-lg hover:bg-primary-50 transition">
              {t.home.cta}
            </Link>
          </motion.div>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((feature, i) => (
            <motion.div key={feature.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
              <Link to={feature.link} className="block p-6 bg-white rounded-xl border border-gray-200 hover:shadow-lg transition h-full">
